...
pub static BUILTINS: &[Builtin] = &[
    ...
    b!("disassemble", 2, Some(2), [Obj, Any] => List; ["object", "verb-desc", "list"]),
    b!("call_function", 1, None, [Str] => Any; ["func-name", "value"]),
    b!("raise", 1, Some(3), [Any, Str, Any]),
    b!("suspend", 0, Some(1), [Int]),
    b!("read", 0, Some(2), [Obj, Any]),
    b!("seconds_left", 0, Some(0), [] => Int),
    b!("ticks_left", 0, Some(0), []),
    b!("pass", 0, None, []),
    b!("set_task_perms", 1, Some(1), [Obj]),
    b!("caller_perms", 0, Some(0), []),
    b!("callers", 0, Some(1), [Any] => List; ["include-line-numbers", "list"]),
    b!("task_stack", 1, Some(2), [Int, Any] => List; ["task-id", "include-line-numbers", "list"]),
    b!("read_stdin", 0, Some(0), []),
    b!("function_info", 0, Some(1), [Str] => List; []),
    b!("load_server_options", 0, Some(0), []),
    b!("value_bytes", 1, Some(1), [Any] => Int; []),
    b!("value_hash", 1, Some(1), [Any] => Str; []),
    b!("string_hash", 1, Some(1), [Str] => Str; ["string", "str"]),
    b!("binary_hash", 1, Some(1), [Str] => Str; ["bin-string", "str"]),
    b!("decode_binary", 1, Some(2), [Str, Any] => List; ["bin-string", "fully", "list"]),
    b!("encode_binary", 0, None, [] => Str; []),
    b!("length", 1, Some(1), [Any] => Int; ["value"]),
    b!("setadd", 2, Some(2), [List, Any]),
    b!("setremove", 2, Some(2), [List, Any]),
    b!("listappend", 2, Some(3), [List, Any, Int] => List; ["list", "value", "index"]),
    b!("listinsert", 2, Some(3), [List, Any, Int] => List; ["list", "value", "index"]),
    b!("listdelete", 2, Some(2), [List, Int] => List; ["list", "index"]),
    b!("listset", 3, Some(3), [List, Any, Int] => List; ["list", "value", "index"]),
    b!("equal", 2, Some(2), [Any, Any] => Int; []),
    ...
    b!("crypt", 1, Some(2), [Str, Str] => Str; ["text", "salt"]),
    b!("index", 2, Some(3), [Str, Str, Any] => Int; ["str1", "str2", "case-matters"]),
    b!("rindex", 2, Some(3), [Str, Str, Any] => Int; ["str1", "str2", "case-matters"]),
    b!("strcmp", 2, Some(2), [Str, Str] => Int; ["str1", "str2", "int"]),
    ...
    b!("encode_chars", 2, Some(2), [Any, Str] => Str; ["arg", "encoding", "bin-string"]),
    b!("decode_chars", 2, Some(3), [Str, Str, Any] => List; ["bin-string", "encoding", "fully", "list"]),
    ...
    b!("min", 1, None, [Numeric] => Numeric),
    b!("max", 1, None, [Numeric] => Numeric),
    b!("abs", 1, Some(1), [Numeric] => Numeric),
...
mod tests {
    ...
    fn builtin_returns_feed_type_inference_and_signatures() {
        ...
        assert_eq!(
            find("index").unwrap().signature(),
            "index(str1: STR, str2: STR, [case-matters: ANY]) -> INT"
        );
        assert_eq!(find("rindex").unwrap().returns, Some(Int));
        for name in ["listappend", "listinsert", "listdelete", "listset"] {
            assert_eq!(find(name).unwrap().returns, Some(List), "{name}");
        }
        assert_eq!(find("crypt").unwrap().returns, Some(Str));
        assert_eq!(find("task_id").unwrap().returns, Some(Int));
        assert_eq!(find("seconds_left").unwrap().returns, Some(Int));
